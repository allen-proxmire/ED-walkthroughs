...
```
α(x^μ): a function of spacetime, locally arbitrary
```
...
```
Ψ(x^μ) → e^(iα(x^μ)/ℏ) Ψ(x^μ)
```

the derivative transforms as

```
∂_μ Ψ → e^(iα/ℏ) [∂_μ Ψ + (i/ℏ)(∂_μ α) Ψ]
```
...
```
A_μ(x^μ) → A'_μ(x^μ) = A_μ(x^μ) − ∂_μ α(x^μ)/q
```
...
```
D'_μ Ψ' = (∂_μ + (iq/ℏ)(A_μ − ∂_μα/q))(e^(iα/ℏ) Ψ)
        = e^(iα/ℏ) [∂_μΨ + (i/ℏ)(∂_μα) Ψ + (iq/ℏ) A_μ Ψ − (i/ℏ)(∂_μα) Ψ]
        = e^(iα/ℏ) [∂_μΨ + (iq/ℏ) A_μ Ψ]
        = e^(iα/ℏ) D_μ Ψ
```
...
```
[D_μ, D_ν] Ψ = (ig/ℏ) F_{μν} Ψ
```

Direct computation gives:

```
F_{μν} = ∂_μ A_ν − ∂_ν A_μ + (ig/ℏ) [A_μ, A_ν]
```
...
```
F_{μν} = ∂_μ A_ν − ∂_ν A_μ        (abelian case)
```

For the non-Abelian case, $F_{\mu\nu}$ becomes Lie-algebra-valued:

```
F_{μν} = F_{μν}^a T^a,    F_{μν}^a = ∂_μ A_ν^a − ∂_ν A_μ^a − (g/ℏ) f^{abc} A_μ^b A_ν^c
```
