...
```
Ψ_transported(x_1) = exp[(iq/ℏ) ∫_γ A_μ dx^μ] · Ψ(x_0)
```
...
```
Ψ_detector ∝ [exp((iq/ℏ) ∫_{γ_1} A_μ dx^μ) + exp((iq/ℏ) ∫_{γ_2} A_μ dx^μ)] · Ψ_source
```
...
```
Ψ_detector ∝ [1 + exp((iq/ℏ) (∫_{γ_2} - ∫_{γ_1}) A_μ dx^μ)] · exp((iq/ℏ) ∫_{γ_1} A_μ dx^μ) · Ψ_source
```

The relative phase between the two paths is:

```
Δφ = (q/ℏ) (∫_{γ_2} - ∫_{γ_1}) A_μ dx^μ = (q/ℏ) ∮_γ A_μ dx^μ
```
...
```
|Ψ_detector|² ∝ |1 + exp(iΔφ)|² = 2(1 + cos Δφ) = 4 cos²(Δφ/2)
```
