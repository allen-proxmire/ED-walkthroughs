...
```
∂_t δ = D F[δ] + H v,    τ v̇ = F[δ] − ζ v,    D + H = 1.
```
...
```
O_0   = δ                      from linear penalty P_0 δ
O_2   = ∇²δ                    from linear mobility M_0 ∇²δ
O_{3,0} = δ³                   from cubic penalty P_3 δ³/6
O_{1,2} = δ²∇²δ                from even mobility M_2 δ² ∇²δ / 2
O_{2,2} = δ |∇δ|²              from even mobility M_2 δ |∇δ|²
O_4   = ∇⁴δ                    higher-derivative (not in bare ED; trial)
O_v   = v                      participation mode
O_{v,t} = v̇                    participation kinetic
O_{δv} = δ v                   two-channel mixing (implicit via H v)
```
...
```
δ(x, t) = δ_<(x, t) + δ_>(x, t),    v(t) = v_<(t) + v_>(t)
```
...
```
x' = x/b,    t' = t/b^z,    δ'(x', t') = b^χ δ_<(bx', b^z t'),    v'(t') = b^{χ_v} v_<(b^z t')
```
...
```
β_{M_0} = 0           (marginal)
β_{P_0} = 2 P_0       (relevant; P_0 grows toward IR)
β_ζ    = 2 ζ          (relevant; ζ grows toward IR)
β_τ    = −2 τ         (irrelevant; τ shrinks toward IR)
β_{P_3} = (2 − 2χ) P_3
β_{M_2} = −2χ M_2
```
...
```
P_0/ζ        invariant (both scale as e^{2ℓ})
P_0 · τ      invariant (one scales as e^{2ℓ}, other as e^{−2ℓ})
ζ · τ        invariant (same combination)
```
...
```
∂_t δ = Γ_eff [M_0 ∇²δ + (M_2/2) δ² ∇²δ + M_2 δ |∇δ|² − P_0 δ − (P_3/6) δ³]
```
...
```
∂_t δ = D M_0 ∇²δ − D P_0(ℓ) δ,    P_0(ℓ) → ∞,    δ → 0
```
...
```
UV: ℓ → −∞                    Intermediate: ℓ_v < ℓ < ℓ_ξ              IR: ℓ → ∞
─────────────────────         ──────────────────────────             ─────────────
Free two-channel             Model A universality                    Linear gapped
diffusion                    Full ED operator structure             diffusion
v unfrozen                   Participation slaved (v = F[δ]/ζ)      Trivial massive
P_0 ≈ 0                     Quadratic-mobility nonlinearities        endpoint δ → 0
M_2, P_3 ≈ 0                Cubic-penalty nonlinearities            Nonlinearities
