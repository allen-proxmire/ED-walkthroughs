...
```
ds²_substrate ≈ ρ² κ_ED² · dτ² + dρ² + (transverse pieces)
```
...
```
N(ω) = 1 / (e^(βω) - 1)   for ω > 0 (bosonic substrate modes)
```
...
```
f_σ(r) → 1 - 2M/r   (Schwarzschild lapse function)
```
...
```
dN/(dt dω) = (1/2π) · Σ_ℓ (2ℓ+1) · 𝒯_ℓ(ω) · N_H(ω)
```
...
```
dM/dt = -(1/2π) Σ_ℓ (2ℓ+1) ∫_0^∞ dω · ω · 𝒯_ℓ(ω) · N_H(ω)
      = -α_Page / M²
```
...
```
N_ED(ω) = N_H(ω) · [1 - (ωτ_V5)² + O((ωτ_V5)^4)]
```
...
- Aggregate remnants from primordial-BH evaporation could contribute to dark matter.
...
- A modified evaporation profile in the late stages of primordial-BH evaporation, possibly producing a stable Planck-mass remnant (cosmologically significant as candidate dark matter).
