...
T17 establishes the substrate gauge field as the participation measure of a structural rule-type τ_g with non-Abelian-capable group content. For a compact simple gauge group G with Lie algebra g of dimension dim(G), write:

```
A_μ(x) = A_μ^a(x) T^a,    a = 1, ..., dim(G)
```
...
```
Φ_μν^a(x) = (V1-kernel-weighted chain transport of A_ν^a along ∂_μ direction)
```
...
Apply hydrodynamic-window coarse-graining ⟨·⟩_R_cg under the scale separation:
...
```
⟨Φ_[μν]^a⟩_R_cg = F_μν^a + O(ℓ_P² ∇²A^a)
```
...
```
∂_μ δΠ_μν^(g) = J_μ^a ∂_μ A_ν^a + (∂_μ J_μ^a) A_ν^a
```
...
```
D_μ J^μ = ∂_μ J^μ,a + g f^abc A_μ^b J^μ,c = 0
```
...
```
(D_μ F^μν)^a = ∂_μ F^μν,a + g f^abc A_μ^b F^μν,c
```
...
```
D_μ F^μν = J^ν,    F_μν^a = ∂_μ A_ν^a − ∂_ν A_μ^a + g f^abc A_μ^b A_ν^c
```
...
```
F_μν^a → F_μν^a + c_V1 ℓ_P² ∇²A_ν^a + ...
```

with c_V1 a dimensionless V1-profile coefficient (proportional to the second-moment integral ⟨δ²⟩_V1 / ℓ_P² of the kernel, inherited at value layer per the standard form-FORCED / value-INHERITED split).
...
| R1: −κμ_V1 ℓ_P² ∇⁴v | YM mass term: c_V1 ℓ_P² ∇²A |
...
```
c_V1 ℓ_P² ∇²A_μ^a → −c_V1 ℓ_P² k² Ã_μ^a
```
...
```
k² → k² + m_eff²(k),    m_eff²(k) ~ c_V1 ℓ_P^(−2)
```
...
```
−(1/4) F_μν^a F^μν,a = −(1/4)(∂_μ A_ν^a − ∂_ν A_μ^a)²
                       − (1/2) g f^abc (∂_μ A_ν^a − ∂_ν A_μ^a) A^μ,b A^ν,c
                       − (1/4) g² (f^abc A_μ^b A_ν^c)²
```
...
```
c_V1(ℓ_P) ℓ_P^(−2) → m_phys² > 0    as ℓ_P → 0
```
...
```
∂_μ F^μν,a + g f^abc A_μ^b F^μν,c = J^ν,a
```
...
```
S_E^kin = (1/4) ∫ d⁴x F_μν^a F^μν,a = (1/2) ∫ d⁴x Tr(F_μν F^μν)
```
...
```
(1/2) Tr(F_μν F^μν) = (1/4) F_μν^a F^μν,a ≥ 0
```
...
```
⟨Θ(J_μ A_μ), (J_μ A_μ)⟩ = ⟨(J_μ A_μ)*, (J_μ A_μ)⟩ ≥ 0
```
