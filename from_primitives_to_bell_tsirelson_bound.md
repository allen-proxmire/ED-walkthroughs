...
Quantum mechanics violates it. Experiments confirm the violation. The standard quantum prediction for a maximally entangled state with optimal measurement angles is |S| = 2√2 ≈ 2.828.

In 1980, Boris Tsirelson proved that 2√2 is not just one quantum prediction — it is the *maximum* value that any quantum correlation can produce. No quantum state, no measurement scheme, no clever protocol can push |S| beyond 2√2. The bound is sharp, the value is exact, and it has been measured in laboratories around the world to within experimental precision.

The Tsirelson bound is one of the deepest empirical structures in physics. It tells us that quantum correlations exceed classical correlations by a specific, finite amount — there is room to violate Bell's inequality, but only so much room. The number 2√2 has no obvious physical interpretation. It does not appear in the formulation of quantum mechanics. It emerges from the mathematics of the bipartite Hilbert space when the CHSH operator is constructed and bounded.

The question this document addresses is: where does 2√2 come from?

In standard quantum mechanics, the answer is: it falls out of the Hilbert-space structure plus the Cauchy-Schwarz inequality plus an operator-norm bound on the CHSH operator. That answer is technically correct, and Tsirelson's 1980 proof remains the canonical derivation. But it leaves the deeper structure unanswered. The Hilbert space is postulated. The inner product that makes Cauchy-Schwarz available is postulated. The bipartite tensor-product structure on H_A ⊗ H_B is postulated. Each of these is a separate axiomatic commitment, and the 2√2 value is what those commitments produce when run through the CHSH operator.

The Event Density framework derives the Tsirelson bound from a smaller set of structural commitments about what reality is made of. The Hilbert-space structure, the inner product, and the bipartite tensor-product structure all emerge as forced consequences of the substrate ontology rather than being postulated. With those structures forced rather than assumed, Tsirelson's 1980 argument applies directly, and the 2√2 value is no longer the output of a chain of postulates but the output of a chain of derived theorems.

The structural shape of this derivation is different from Born or Schrödinger. Born had T14's Cauchy argument as a substantive new derivation — the framework genuinely earns the squared exponent through a primitive-level argument. Schrödinger had the Galilean integration producing the factor of 1/(2m) — again a substantive new derivation step. Bell-Tsirelson is different. The framework's contribution here is upstream: it forces the Hilbert-space structure that Tsirelson's argument operates on. The argument itself, including the operator-norm bound that produces 2√2, is mathematical physics that's already in the literature. What changes is the foundational status of the structures that argument requires.

That's worth being honest about. The Tsirelson bound is not a place where ED produces a new derivation step beyond standard quantum mechanics. It is a place where ED removes the postulational status of the structures that the standard derivation already uses. The 2√2 value is now a derived consequence of the substrate ontology, all the way down.
...
3. The bipartite tensor-product structure on H_A ⊗ H_B is forced by extending U2 to multi-system arrangements — same primitive-level arguments, applied to the joint participation graph of separated chains.

4. With the Hilbert-space structure forced, Tsirelson's argument applies: Cauchy-Schwarz on the inner product plus operator-norm bounds on the CHSH operator produces |S| ≤ 2√2 as the maximum quantum correlation.

Steps 1 and 2 are carried over from prior walkthroughs. Step 3 is new but lightweight — it extends U2's primitive-level arguments to bipartite systems without introducing new structural commitments. Step 4 is the standard Tsirelson argument, walked here for completeness because it's where the 2√2 value actually emerges.

The structural payoff: |S| ≤ 2√2 is what falls out of Cauchy-Schwarz applied to the inner product on the bipartite participation-measure Hilbert space, evaluated against the CHSH operator. The bound is a consequence of inner-product geometry plus operator algebra, both of which are derived structures in the framework rather than postulates.
...
The participation measure form, P_K = √b_K · e^(iπ_K), forced by T14. The Cauchy functional equation on bandwidth additivity fixes the square root; Frobenius's theorem on real division algebras fixes the complex-valued phase.
...
To talk about quantum correlations, we need a Hilbert space that describes the joint state of Alice's and Bob's systems together. In standard quantum mechanics, this is constructed as the tensor product H_A ⊗ H_B of the two single-system Hilbert spaces. The construction is postulated — it's what the standard formalism uses for composite systems.
...
```
P_{K_A, K_B}(x_A, x_B) = √b_{K_A, K_B}(x_A, x_B) · e^(iπ(K_A, K_B, x_A, x_B))
```
...
The joint participation-measure space P_AB is the set of all such complex-valued arrays indexed by (K_A, K_B) channel pairs and joint vertex configurations.
...
**Linearity.** P_AB is a complex vector space because the joint participation measures take values in C, and complex-valued function spaces inherit complex-vector-space structure pointwise. Same argument as the single-system case.
...
```
⟨P | Q⟩_AB = Σ_{K_A, K_B} Σ_{u_A, u_B} P*_{K_A, K_B}(u_A, u_B) · Q_{K_A, K_B}(u_A, u_B)
```
...
The bipartite Hilbert space H_AB constructed this way is naturally isomorphic to the tensor product H_A ⊗ H_B. The isomorphism is the standard one: a joint state P_{K_A, K_B}(u_A, u_B) that factors as a product P^A_{K_A}(u_A) · P^B_{K_B}(u_B) corresponds to the tensor product |P^A⟩ ⊗ |P^B⟩, and general joint states are linear combinations of such factored states.

The inner product respects the tensor-product structure:

```
⟨P^A ⊗ P^B | Q^A ⊗ Q^B⟩_AB = ⟨P^A | Q^A⟩_A · ⟨P^B | Q^B⟩_B
```

This follows directly from the bipartite inner product's local pointwise structure: the joint product P*_{K_A, K_B}(u_A, u_B) · Q_{K_A, K_B}(u_A, u_B) factorizes when both states factorize, and the sums over (K_A, K_B) and (u_A, u_B) decouple into separate sums over A and B variables.

Entangled states — states that don't factorize as P^A ⊗ P^B — exist in H_AB exactly because the bipartite participation graph admits joint states that are not products of single-chain states. The framework doesn't forbid entanglement; it accommodates entanglement structurally as the natural consequence of joint participation graphs supporting non-factorizable bandwidth assignments.

### 3.4 What this delivers

The bipartite Hilbert space H_AB = H_A ⊗ H_B is now derived from the same primitive-level arguments that derived the single-system Hilbert space. No new structural commitment was introduced — U2's machinery extended to the joint system gives the tensor-product structure as a forced consequence.

Operators on H_AB include local operators of the form Â ⊗ I_B (acting only on Alice's system) and I_A ⊗ B̂ (acting only on Bob's). Bipartite operators that mix the two systems, like Â_1 ⊗ B̂_1 + Â_2 ⊗ B̂_2, are linear combinations of these tensor products. The CHSH operator that drives Tsirelson's argument is built from such combinations.
...
Alice has two binary measurement settings, denoted A and A', each producing outcomes ±1. Bob has two binary measurement settings, B and B', each producing outcomes ±1. The four settings give four possible correlation experiments: (A, B), (A, B'), (A', B), (A', B').
...
Bell's inequality, derived under the assumption of local hidden variables, states that |S| ≤ 2 for any local realist theory.

### 4.2 The CHSH operator in quantum mechanics

In quantum mechanics, each measurement setting is represented by a self-adjoint operator with eigenvalues ±1. Alice's settings are represented by Â and Â' on H_A; Bob's settings by B̂ and B̂' on H_B. The correlation E(X̂, Ŷ) for a joint state |ψ⟩ ∈ H_AB is:

```
E(X̂, Ŷ) = ⟨ψ| X̂ ⊗ Ŷ |ψ⟩
```

The CHSH operator on H_AB is:

```
Ŝ = Â ⊗ B̂ + Â ⊗ B̂' + Â' ⊗ B̂ - Â' ⊗ B̂'
```

The CHSH expression S = ⟨ψ|Ŝ|ψ⟩ is the expectation value of this operator in the joint state |ψ⟩.

Note what's happening here. The CHSH operator is built from tensor products of single-system operators, summed and subtracted in the specific pattern Bell's analysis requires. The operator lives on H_AB. Computing ⟨S⟩ uses the bipartite inner product. The whole construction relies on the tensor-product Hilbert-space structure that Section 3 established.
...
for any normalized state |ψ⟩ ∈ H_AB and any choice of self-adjoint ±1-eigenvalued operators Â, Â', B̂, B̂'.

To prove this, we need to show that the operator norm of Ŝ is at most 2√2:

```
‖Ŝ‖ ≤ 2√2
```

because then |⟨ψ|Ŝ|ψ⟩| ≤ ‖Ŝ‖ for any normalized state.
...
The operator-norm bound ‖Ŝ‖ ≤ 2√2 is a result about self-adjoint operators on Hilbert spaces with specific algebraic properties. The derivation uses Cauchy-Schwarz, the spectral theorem, and the algebra of tensor products. None of these are framework-specific — they're standard mathematical physics that operates on the Hilbert-space structure regardless of how that structure was obtained.

Walking the argument here is worth doing because the 2√2 value emerges in a specific way, and understanding where it comes from is part of understanding what the bound means.

### 5.1 Squaring the CHSH operator

Tsirelson's strategy is to compute Ŝ² and bound it using operator algebra.
...
Each of Â, Â', B̂, B̂' is self-adjoint with eigenvalues ±1, so each squares to the identity:
...
```
[Â ⊗ I, I ⊗ B̂] = 0,   [Â' ⊗ I, I ⊗ B̂'] = 0,  etc.
```

But Â and Â' do not generally commute on H_A, and similarly for B̂ and B̂' on H_B. Their commutators [Â, Â'] and [B̂, B̂'] are non-trivial in general.

After expanding and using the squaring identities for the diagonal terms, the result is:

```
Ŝ² = 4 I_A ⊗ I_B - [Â, Â'] ⊗ [B̂, B̂']
```

The diagonal terms each contribute Â² ⊗ B̂² = I ⊗ I, and there are four of them, giving 4 I ⊗ I. The cross terms collect into the commutator product [Â, Â'] ⊗ [B̂, B̂'] with the right sign structure from the +,+,+,- pattern of the CHSH expression.

This is a remarkable identity. The square of the CHSH operator decomposes into a constant 4 I ⊗ I plus a term involving the commutators of the local operators. If the local operators commuted (Â with Â', B̂ with B̂'), the commutator term would vanish and ‖Ŝ²‖ = 4, giving ‖Ŝ‖ = 2 — the classical Bell bound. The quantum violation comes from the non-commuting local operators.

### 5.2 Bounding the commutator term

The norm of [Â, Â'] is bounded using the fact that Â and Â' are self-adjoint with eigenvalues ±1. The commutator [Â, Â'] = ÂÂ' - Â'Â is anti-Hermitian, and its norm satisfies:

```
‖[Â, Â']‖ ≤ ‖ÂÂ'‖ + ‖Â'Â‖ ≤ 2 ‖Â‖ ‖Â'‖ = 2
```

since ‖Â‖ = ‖Â'‖ = 1 (eigenvalues ±1 means the operator norm is 1). Similarly, ‖[B̂, B̂']‖ ≤ 2.

For the tensor product:

```
‖[Â, Â'] ⊗ [B̂, B̂']‖ = ‖[Â, Â']‖ · ‖[B̂, B̂']‖ ≤ 2 · 2 = 4
```
...
```
‖Ŝ²‖ ≤ ‖4 I ⊗ I‖ + ‖[Â, Â'] ⊗ [B̂, B̂']‖ ≤ 4 + 4 = 8
```

For a self-adjoint operator (which Ŝ is, as a real linear combination of self-adjoint tensor products):
...
This is the Tsirelson bound. For any normalized state |ψ⟩ ∈ H_AB:

```
|⟨ψ|Ŝ|ψ⟩| ≤ ‖Ŝ‖ ≤ 2√2
```

The 2√2 value emerges from two combined facts: the diagonal terms in Ŝ² contribute 4 (because there are four ±1-eigenvalued operators each squaring to I), and the operator-norm bound on the commutator product contributes another 4. The square root produces 2√2.

### 5.4 The bound is tight

The 2√2 bound is sharp — there exist quantum states that saturate it. The maximally entangled Bell state:

```
|ψ⟩ = (1/√2)(|00⟩ + |11⟩)
```

with measurement angles chosen at 45° offsets between Alice and Bob, achieves |S| = 2√2 exactly. This is what experiments measure when they violate Bell's inequality: a value approaching 2√2 from below, limited only by experimental noise and detector efficiency.
...
The bipartite Hilbert space H_AB with its tensor-product structure (Section 3).

The inner product ⟨ψ|Ŝ|ψ⟩ that lets us compute expectation values (U2).

The operator norm ‖Â‖ on each Hilbert space, which the inner product induces.

The spectral theorem for self-adjoint operators with eigenvalues ±1, which gives the squaring identity Â² = I.

The tensor-product algebra that lets us compute (Â ⊗ B̂)² = Â² ⊗ B̂² and ‖Â ⊗ B̂‖ = ‖Â‖ · ‖B̂‖.
...
The 2√2 bound is therefore a derived consequence of the substrate ontology, all the way down. The structures Tsirelson's argument requires are now forced rather than postulated.
...
That's an honest accounting. The Tsirelson bound is not a place where ED produces a new derivation step beyond standard quantum mechanics. It is a place where ED removes the postulational status of the structures the standard derivation already uses. For a reader who's already convinced that 2√2 is the right answer and just wants to know why the underlying machinery is valid, this is what the framework offers: the machinery is no longer an article of faith. Every piece of the derivation traces back to substrate primitives.

For a reader interested in the deeper question of *why* quantum correlations exceed classical correlations by exactly this much — why 2√2 rather than some other value — the framework's answer is the same as standard quantum mechanics: because Cauchy-Schwarz on a Hilbert space, applied to operators with the algebraic properties that ±1-eigenvalued self-adjoint operators have, on a tensor-product Hilbert space, produces this specific bound through this specific argument. The 2√2 comes from the operator algebra, not from ED-specific structural commitments. ED ensures the Hilbert space is real; what happens on it is then standard mathematics.
...
Primitives (micro-events, participation, channels, bandwidth, polarity, commitment) → T14 (participation measure form forced) → U2 (inner product on single-system space forced) → bipartite extension of U2 (tensor-product Hilbert space H_AB forced) → CHSH operator construction on H_AB (standard quantum mechanics) → Tsirelson's argument: Ŝ² = 4 I + commutator term, operator-norm bound 2 on commutators, squared norm bound 8 on Ŝ², square root gives ‖Ŝ‖ ≤ 2√2.

The 2√2 bound is now a derived consequence of substrate ontology rather than a consequence of postulated Hilbert-space structure. The mathematical content of Tsirelson's 1980 proof is unchanged — what changes is the foundational status of the Hilbert space the proof operates on.

The framework reproduces Bell-Tsirelson exactly. It does not predict any new violation pattern, any new bound, or any deviation from standard quantum correlations in regimes where standard quantum mechanics has been tested. Quantum correlations are bounded by 2√2 in the framework just as in standard QM. What changes is that the bound is now traceable to the substrate primitives without invoking the Hilbert-space postulate.

For experimentally verified Bell-test violations, every value measured in laboratories — from Aspect's 1981 experiment through the loophole-free tests of 2015 and beyond — sits at or below 2√2 within experimental precision. The framework predicts the same. The Tsirelson bound is not a place where ED differs from standard quantum mechanics; it's a place where ED derives the structures that produce the bound rather than assuming them.
