...
```
ρ̂_{KK'}(x, x', t) = ⟨P_K(x, t) P_{K'}^*(x', t)⟩_env
```
...
```
∂_t ρ̂ = Σ_j p_j (∂_t|Ψ_j⟩⟨Ψ_j| + |Ψ_j⟩∂_t⟨Ψ_j|)
       = Σ_j p_j (-(i/ℏ)Ĥ|Ψ_j⟩⟨Ψ_j| + (i/ℏ)|Ψ_j⟩⟨Ψ_j|Ĥ)
       = -(i/ℏ)(Ĥρ̂ - ρ̂Ĥ)
       = -(i/ℏ)[Ĥ, ρ̂]
```
...
```
⟨ρ̂(t + dt)⟩_jumps = Σ_α (λ_α dt) · (Ĵ_α ρ̂ Ĵ_α† / λ_α) = Σ_α Ĵ_α ρ̂ Ĵ_α† dt
```
...
```
ρ̂(t + dt) = (1 − Σ_α λ_α dt) · ρ̂_{no-commit}(t + dt) + Σ_α (λ_α dt) · ρ̂_{commit-α}(t)
```
...
```
Tr(ρ̂(t + dt)) = (1 − Σ_α λ_α dt) · Tr(ρ̂_{no-commit}) + Σ_α (λ_α dt) · Tr(ρ̂_{commit-α}) = 1
```
...
```
d/dt Tr(ρ̂) = Tr(-(i/ℏ)[Ĥ, ρ̂]) + Σ_α Tr(Ĵ_α ρ̂ Ĵ_α†) − (1/2) Σ_α Tr({Ĵ_α† Ĵ_α, ρ̂})
            = 0 + Σ_α Tr(Ĵ_α† Ĵ_α ρ̂) − Σ_α Tr(Ĵ_α† Ĵ_α ρ̂)
            = 0
```
...
```
∂_t ρ̂ = -(i/ℏ)[Ĥ, ρ̂] + Σ_α Ĵ_α ρ̂ Ĵ_α† − (1/2) Σ_α {Ĵ_α† Ĵ_α, ρ̂}
```
