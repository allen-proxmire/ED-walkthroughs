...
3. The configuration-space topology π_1(Q_2) = ℤ_2 (R.2.3, first half)
...
**3+1D spatial commitment.** The substrate is committed to three spatial dimensions plus one temporal dimension. This is a substrate-level commitment, not a derivation. The walkthrough's topological arguments depend on this: π_1(Q_2) = ℤ_2 is specific to 3+1D — the same result fails in 2+1D where π_1(Q_2) = ℤ (allowing anyons) and is trivial in 4+1D and higher.

**Lorentz-covariant participation measure.** From the Klein-Gordon walkthrough, the participation measure is:

```
P_K(x^μ) = √b_K(x^μ) · e^(iπ_K(x^μ))
```
...
**Two-chain configuration space.** For two same-type chains K_A and K_B with worldlines γ_A and γ_B, the relevant configuration space at a fixed time slice is:

```
Q_2 = (ℝ³ × ℝ³ \ Δ) / S_2
```

where ℝ³ × ℝ³ is the unrestricted two-chain position space, Δ is the diagonal (where the two chains coincide, removed because individuation forbids strict coincidence for indistinguishable chains), and S_2 is the symmetric group on two elements quotienting by the chain-label exchange. The fundamental group π_1(Q_2) catalogs the topologically distinct paths a two-chain configuration can take in this space.
...
The third structural move establishes π_1(Q_2) = ℤ_2 from the geometric structure of the two-identical-chain configuration space in 3+1D. This is the topological argument that simultaneously forbids anyons and identifies the exchange-class generator with the 2π-rotation generator.
...
The framework's job is to compute π_1(Q_2), the fundamental group cataloging topologically distinct loops in this configuration space.
...
Combining: π_1(Q_2) = π_1((ℝ³ \ {0})/ℤ_2) = π_1(ℝP²) = ℤ_2.
...
Since π_1(Q_2) = ℤ_2 has only two elements, the only 1-dimensional unitary representations of π_1(Q_2) are:
...
In particular, anyonic exchange phases η = e^(iθ) for arbitrary θ are forbidden in 3+1D. Anyons require π_1(Q_2) = ℤ rather than ℤ_2, which occurs in 2+1D where the relative configuration space deformation-retracts to S¹ rather than ℝP² and π_1(S¹) = ℤ. The framework's prediction matches: in 2+1D systems (effectively two-dimensional condensed-matter systems like the fractional quantum Hall effect), anyons are observed; in 3+1D, they are not.
...
**The theorem.** Under the SO(3)-equivariant embedding of the relative coordinate, the exchange-path generator (the loop γ_E that exchanges the two chains, generating π_1(Q_2)) is homotopic to a 2π-rotation of R.

**Geometric argument.** A path that exchanges the two chains takes R(0) at the "north pole" of S² to R(1) = −R(0) at the "south pole." On ℝP² (the quotient by antipodal identification), this is a closed path generating π_1(ℝP²). Lifting back to S², this path is half of a great circle through the north and south poles. To close the loop on ℝP², we identify R(1) ∼ −R(1), but on S² this corresponds to a continuous rotation through 2π (after which R returns to its starting orientation in the lift).

The exchange-path generator is therefore the 2π-rotation generator. They are the same element of π_1(Q_2) = ℤ_2.
...
For a rotation by angle θ around the z-axis, ω_12 = θ and the spinor transformation is:

```
S(R_z(θ)) = exp(−(i/2)θ σ^12)
```

For θ = 2π:

```
S(R_z(2π)) = exp(−iπ σ^12) = cos(π) · 𝟙 − i sin(π) σ^12 = −𝟙
```
...
π_1(Q_2) = ℤ_2 with the geometric theorem identifying exchange-class generator with 2π-rotation generator from R.2.3.
...
(C) Configuration-space topology π_1(Q_2) = ℤ_2 in 3+1D (R.2.3, first half).
...
```
B(P^A, P^B) → η · B(P^B, P^A) = η · B̃(P^A, P^B)
```
...
The geometric theorem (D) says the exchange operation generates the same element of π_1(Q_2) as 2π-rotation. The bilinear B must therefore transform under 2π-rotation in the same way it transforms under exchange.
...
```
B → D(R(2π))^† · Γ · D(R(2π)) · B = (−1) · Γ · (−1) · B = Γ · B
```
...
```
B → D(R(2π)) acting on one chain's module · B = (−1) · B
```
...
Anyons forbidden in 3+1D (from π_1(Q_2) = ℤ_2 having only two elements).
