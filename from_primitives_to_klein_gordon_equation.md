...
Dimensional analysis fixes M² = m²c²/ℏ² up to a dimensionless factor, with m the chain's mass, ℏ inherited from the QM-emergence sector, and c the speed of light.

Plane-wave application gives the mass-shell condition p_μp^μ = m²c², equivalently E² = p²c² + m²c⁴.

Local U(1) gauge invariance forces the gauge-covariant derivative D_μ = ∂_μ + (iq/ℏ)A_μ, producing the interacting Klein-Gordon equation and the conserved four-current.

The non-relativistic limit, via rest-energy factorization Ψ = e^(−imc²t/ℏ)ψ, recovers the free-particle Schrödinger equation exactly. With electromagnetic coupling included, it recovers the Schrödinger equation with magnetic and scalar potentials.
...
a Lorentz-scalar field. The event density ρ(x^μ) = Σ_K |P_K(x^μ)|² is a Lorentz scalar. These are the natural relativistic generalizations of the Phase-1 wavefunction and event density.
...
In the relativistic reformulation, each of the four bands becomes a Lorentz-scalar field on spacetime: b_K^int(x^μ), b_K^adj(x^μ), b_K^env(x^μ), b_K^com(x^μ). The decomposition and the sum-rule b_K = Σ_i b_K^(i) are Lorentz-invariant. The four bands carry Lorentz-scalar content at each spacetime event.
...
For coherent channel structure within a single chain, the relational timing is the phase advancement ∂_τπ_K per proper-time interval — Lorentz-invariant. This is forced by Lorentz invariance: proper time is the natural Lorentz-invariant parameter.
...
A natural worry: the Schrödinger equation iℏ∂_tΨ = ĤΨ is first-order in time. Why does the relativistic version need to be second-order?
...
```
p_μx^μ = (E/c) · ct − (−**p**) · **x** = Et − **p**·**x**
```

so:

```
P_K(x^μ) = c_K · e^(i(**p**·**x** − Et)/ℏ)
```
...
```
□P_K = ∂_μ∂^μ (c_K · e^(−ip_νx^ν/ℏ))
```

Using ∂_μ e^(−ip_νx^ν/ℏ) = (−ip_μ/ℏ) e^(−ip_νx^ν/ℏ):

```
□P_K = c_K · (−ip_μ/ℏ)(−ip^μ/ℏ) · e^(−ip_νx^ν/ℏ)
       = −(p_μp^μ/ℏ²) · P_K
```
...
The constant M² has units of [length]^(−2). The unique combination of the framework's dimensional anchors — the chain mass m, the speed of light c, and ℏ — with units of [length]^(−2) is:
...
```
Ψ(x^μ) = ∫ d⁴p/(2π)⁴ · (2πℏ) δ(p_μp^μ − m²c²) · ã(p) · e^(−ip_μx^μ/ℏ)
```
...
The participation measure has a global U(1) symmetry: the phase π_K(x^μ) can be shifted by a constant α/ℏ, which multiplies the wavefunction by an overall phase factor e^(iqα/ℏ):

```
Ψ(x^μ) → e^(iqα/ℏ) Ψ(x^μ)
```

(The factor of q/ℏ in the phase is conventional — it sets the natural scale.)
...
```
Ψ(x^μ) → Ψ'(x^μ) = e^(iqα/ℏ) Ψ(x^μ)
A_μ(x^μ) → A'_μ(x^μ) = A_μ − ∂_μα
```

Direct computation:

```
D'_μ Ψ' = (∂_μ + (iq/ℏ)A'_μ)(e^(iqα/ℏ) Ψ)
        = e^(iqα/ℏ) [∂_μΨ + (iq/ℏ)(∂_μα)Ψ + (iq/ℏ)(A_μ − ∂_μα)Ψ]
        = e^(iqα/ℏ) [∂_μΨ + (iq/ℏ)A_μΨ]
        = e^(iqα/ℏ) D_μΨ
```
...
```
D_μD^μ = (∂_μ + (iq/ℏ)A_μ)(∂^μ + (iq/ℏ)A^μ)
       = □ + (iq/ℏ)[A_μ∂^μ + ∂_μ(A^μ ·)] − (q²/ℏ²)A_μA^μ
```

In Lorenz gauge ∂_μA^μ = 0, this simplifies to:

```
D_μD^μΨ = □Ψ + (2iq/ℏ)A_μ∂^μΨ − (q²/ℏ²)A_μA^μ Ψ
```
...
Higher-order modifications (terms proportional to A_μA_νΨ at first order in derivatives, or non-minimal couplings involving curvature-like field strengths) are also gauge-covariant in principle but introduce additional structure that's not forced by local invariance alone. The minimal-coupling prescription D_μ = ∂_μ + (iq/ℏ)A_μ is the unique first-order, minimum-structure solution.
...
The gauge-covariant derivative D_μ = ∂_μ + (iq/ℏ)A_μ is forced by local U(1) gauge invariance. The interacting Klein-Gordon equation (D_μD^μ + m²c²/ℏ²)Ψ = 0 is the unique first-order gauge-covariant extension of the free equation.
...
```
j^μ(x^ν) = (iℏ/2m)[Ψ* D^μΨ − Ψ(D^μΨ)*]
```

The factor of i and the antisymmetric combination Ψ*(D^μΨ) − Ψ(D^μΨ)* make j^μ a real four-vector (the bracketed quantity is purely imaginary, multiplied by i gives a real number, and ℏ/(2m) is real).

### 7.2 Gauge invariance

Under a local U(1) transformation, Ψ → e^(iqα/ℏ)Ψ and D_μΨ → e^(iqα/ℏ)D_μΨ. The bilinear Ψ*(D^μΨ) transforms as:

```
Ψ*(D^μΨ) → (e^(−iqα/ℏ)Ψ*)(e^(iqα/ℏ)D^μΨ) = Ψ*(D^μΨ)
```
...
```
∂_μ(Ψ*D^μΨ) − ∂_μ(Ψ(D^μΨ)*) = (D_μΨ)*(D^μΨ) − (D_μΨ)(D^μΨ)*
```
...
The four-current has components j^μ = (cρ_em, **j**_em) where ρ_em is the electric charge density and **j**_em is the charge-current density. The continuity equation in components reads:
...
The zeroth component j^0 of the Klein-Gordon current is not positive-definite. For a stationary plane-wave mode Ψ = e^(−iEt/ℏ) with positive energy E > 0, the time component j^0 is positive. But for a negative-energy mode Ψ = e^(+i|E|t/ℏ), j^0 is negative.
...
```
Ψ(x^μ) = e^(−iE_0t/ℏ) · ψ(**x**, t) = e^(−imc²t/ℏ) · ψ(**x**, t)
```
...
```
∂_tΨ = e^(−imc²t/ℏ) [−(imc²/ℏ)ψ + ∂_tψ]

∂_t²Ψ = e^(−imc²t/ℏ) [−(m²c⁴/ℏ²)ψ − (2imc²/ℏ)∂_tψ + ∂_t²ψ]
```

Dividing by c²:

```
(1/c²)∂_t²Ψ = e^(−imc²t/ℏ) [−(m²c²/ℏ²)ψ − (2im/ℏ)∂_tψ + (1/c²)∂_t²ψ]
```
...
Substituting and dividing through by e^(−imc²t/ℏ):

```
−(m²c²/ℏ²)ψ − (2im/ℏ)∂_tψ + (1/c²)∂_t²ψ − ∇²ψ + (m²c²/ℏ²)ψ = 0
```

The (m²c²/ℏ²)ψ terms cancel — the rest-energy factor was chosen precisely to make this happen. The remaining equation:

```
−(2im/ℏ)∂_tψ + (1/c²)∂_t²ψ − ∇²ψ = 0
```

### 8.4 Dropping the relativistic correction

The term (1/c²)∂_t²ψ is suppressed relative to (2im/ℏ)∂_tψ in the non-relativistic regime. To see this: if the characteristic energy scale of ψ is the kinetic energy E_kin ~ p²/(2m) ≪ mc², then:

```
(1/c²)∂_t²ψ ~ E_kin²/(ℏ²c²) · ψ
(2im/ℏ)∂_tψ ~ 2mE_kin/ℏ² · ψ
```
...
The non-relativistic limit of Klein-Gordon, via rest-energy factorization Ψ = e^(−imc²t/ℏ)ψ and dropping the suppressed (1/c²)∂_t² term, recovers the free-particle Schrödinger equation iℏ∂_tψ = −(ℏ²/2m)∇²ψ exactly. With electromagnetic coupling, it recovers the Schrödinger equation with magnetic and scalar potentials.
...
The Lorentz-covariant participation measure P_K(x^μ) = √b_K · e^(iπ_K) on Minkowski spacetime, transforming as a scalar under SO⁺(3,1) for bosonic chains.
...
The gauge-covariant derivative D_μ = ∂_μ + (iq/ℏ)A_μ as the unique first-order modification restoring local U(1) gauge invariance.
...
The reduced Planck constant ℏ (inherited via U3 from the Phase-1 framework, as in the Schrödinger walkthrough).
...
Primitives (micro-events, participation, chains, channels, bandwidth, polarity, commitment irreversibility, relational timing) → Lorentz-covariant reformulation (chain → worldline, ED gradient → four-gradient, relational timing → proper-time, four-band → covariant scalar fields) → Lorentz-covariant participation measure P_K(x^μ) = √b_K · e^(iπ_K) → unique second-order Lorentz-scalar operator □ → minimum-order linear scalar equation (□ + M²)Ψ = 0 → dimensional analysis fixes M² = m²c²/ℏ² → mass-shell condition from plane-wave application → relativistic dispersion E² = p²c² + m²c⁴ → local U(1) invariance forces D_μ = ∂_μ + (iq/ℏ)A_μ → interacting Klein-Gordon equation → conserved four-current → non-relativistic limit recovers Schrödinger.
