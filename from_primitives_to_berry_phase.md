...
*A walkthrough-grade ED mini-Arc deriving the Berry phase, Berry connection, and Berry curvature from substrate primitives. Fully self-contained: all required math is derived inside this document.*
...
```
|ψ(R)⟩ = Σ_n α_n(R) |r_n(R)⟩,    Σ_n |α_n(R)|² = 1.
```
...
```
T  ≫  ℏ / Δ_{nm}(R),    where Δ_{nm}(R) = E_n(R) - E_m(R)  (m ≠ n)
```
...
```
ċ_m(t) = -Σ_n c_n(t) e^{i(φ_m - φ_n)} ⟨m(R(t))|d/dt|n(R(t))⟩.
```
...
```
(∇_R H) |n⟩ + H |∇_R n⟩ = (∇_R E_n) |n⟩ + E_n |∇_R n⟩.
```

Project onto $\langle m|$ for $m \neq n$:

```
⟨m|∇_R H|n⟩ + ⟨m|H|∇_R n⟩ = (∇_R E_n) ⟨m|n⟩ + E_n ⟨m|∇_R n⟩.
```

Using $\langle m|H = E_m\langle m|$ and $\langle m|n\rangle = 0$:

```
⟨m|∇_R H|n⟩ + E_m ⟨m|∇_R n⟩ = E_n ⟨m|∇_R n⟩,
```

which gives

```
⟨m|∇_R n⟩  =  ⟨m|∇_R H|n⟩ / (E_n - E_m)    for m ≠ n.
```
...
```
γ_n(T) - γ_n(0)  =  ∫_0^T A_n(R(t)) · Ṙ(t) dt
                 =  ∫_C A_n(R) · dR,
```
...
```
⟨ψ_PT|d/dt|ψ_PT⟩  =  iγ̇_n + ⟨n|d/dt|n⟩  =  iγ̇_n - i A_n · Ṙ  =  0.
```
...
```
|ψ(T)⟩  =  e^{-iφ_n(T)} e^{iγ_n(T)} |n(R(T))⟩  =  e^{-iφ_n(T)} e^{iγ_n(T)} |n(R_0)⟩.
```
...
```
γ_n[C]  =  ∮_C A_n(R) · dR,    A_n(R) = i ⟨n(R)|∇_R n(R)⟩.
```
...
```
Ã_n(R)  =  i ⟨ñ(R)|∇_R ñ(R)⟩
       =  i ⟨n(R)| e^{-iχ(R)} ∇_R [e^{iχ(R)} |n(R)⟩]
       =  i ⟨n(R)| e^{-iχ(R)} [i (∇_R χ) e^{iχ(R)} |n(R)⟩ + e^{iχ(R)} |∇_R n(R)⟩]
       =  -⟨n(R)|n(R)⟩ ∇_R χ  +  i ⟨n(R)|∇_R n(R)⟩
       =  A_n(R) - ∇_R χ(R)
```
...
```
γ̃_n[C]  =  ∮_C Ã_n · dR  =  ∮_C A_n · dR - ∮_C ∇_R χ · dR  =  γ_n[C] - [χ(R(T)) - χ(R(0))].
```
...
```
Ω_n(R)  =  ∇_R × A_n(R)         (in three-dimensional parameter space)
```

or, more generally, as the two-form

```
Ω_n  =  d A_n  =  (∂_i A_{n,j} - ∂_j A_{n,i}) dR^i ∧ dR^j / 2,
```
...
```
Ω̃_n  =  d(A_n - dχ)  =  d A_n - d²χ  =  d A_n  =  Ω_n,
```
...
```
∮_C A_n · dR  =  ∬_S (∇_R × A_n) · dS  =  ∬_S Ω_n · dS,
```
...
```
∬_{S_1} Ω_n · dS - ∬_{S_2} Ω_n · dS  =  ∮_{∂V} A_n · dR  - ∮_{∂V} A_n · dR  =  0,
```
