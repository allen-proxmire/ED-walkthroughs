# From Primitives to the Kernel-Level Arrow of Time
...
But the fundamental laws of physics are time-symmetric. Newton's equations look the same running forward or backward. Maxwell's equations have no preferred time direction. The Schrödinger equation, the Klein-Gordon equation, the Dirac equation, general relativity, the Standard Model — all of them admit time-reversal as a symmetry of their dynamics. If you filmed a quantum system evolving according to the Schrödinger equation and ran the film backward, the backward evolution would also be a valid quantum system evolving according to the Schrödinger equation.
...
That said, the kernel-level result is significant on its own terms. The standard view — that the dynamics are time-symmetric and the arrow comes from boundary conditions — is partly displaced if the framework's argument holds. The dynamics, properly understood at the substrate level, are *not* time-symmetric. The vacuum response to chain perturbations is forward-cone-only by structural necessity. The time-symmetry of standard QFT is an emergent feature of the continuum approximation, not a fundamental property of the substrate.

The chain has four steps:

1. The primitive of commitment irreversibility (P11) plus a few supporting primitives jointly force chain bandwidth dynamics to be forward-only along proper time.

2. The vacuum response kernel V1 is forced to have finite-width structure by independent arguments (UV-FIN plus event-discreteness primitives). V1 exists as a finite-width object before the question of directionality is asked.

3. V1 is a response kernel — not a correlator. As a response kernel, V1 is sourced by chain bandwidth content via vacuum-coupling structure. The chain-contribution construction makes this explicit.

4. Forward-only chain bandwidth dynamics plus response-kernel structure jointly force V1 to have forward-cone-only support. Symmetric and advanced alternatives are refuted: symmetric V1 cannot be constructed from forward-only chain dynamics; advanced V1 directly contradicts P11.
...
**Micro-events.** Reality consists of discrete acts of becoming. Each micro-event is a vertex in a graph spanning the event manifold. Events are discrete at the substrate level; there is a primitive-level event-resolution scale ℓ_ED below which the manifold structure is not continuum-like.

**Participation.** Micro-events don't exist in isolation; they participate in one another's becoming. The participation relation is the edge structure of the graph.

**Bandwidth.** The graded measure of participation, supplied as a non-negative real edge weight. Bandwidth admits a four-band orthogonal decomposition (internal-rule, adjacency, environmental, commitment-reserve) with conservation along chains.

**Chains.** Stable sequences of commitment events along which a chain's update rule is repeatedly instantiated. Chains are the substrate-level objects the framework calls "particles." Each chain has a worldline γ_K parameterized by proper time τ_K.

**Commitment.** The discrete event in which a chain selects one channel from those available at a vertex. This is where the load-bearing primitive enters.

**P11 — Commitment irreversibility.** Every commitment event is irreversible. Once a commitment is made, it is never unmade. There is no "un-commitment" operation in the substrate; commitment events along γ_K are ordered forward along proper time, and the ordering admits no reversal.
...
**Relational timing.** Supplies the time axis along which chain proper time τ_K is parameterized. Combined with P11, this gives chain commitment events a forward-ordered structure: τ_K increases monotonically along γ_K, with later commitment events having larger τ_K than earlier ones.

**ED gradient.** Supplies the spatial axis structure. Combined with relational timing, gives the (3,1) signature of the substrate's emergent geometry.
...
## 3. Forcing Forward-Only Chain Dynamics
...
### 3.1 P11 alone is not enough

P11 says every commitment event is irreversible. By itself, this is a fact about chain commitment structure: commitments are not undone. But P11 alone does not specify how bandwidth dynamics evolve along a chain. A priori, one could imagine commitment events being irreversible while bandwidth content at each commitment event is determined by some bidirectional rule — bandwidth could in principle be a function of past *and* future commitment events, even with the commitments themselves being irreversible.

The argument must show that P11 plus other primitives jointly force bandwidth to be forward-only — that the bandwidth content at commitment event τ_K^(n) is determined by the chain's past history (commitment events with τ_K^(m) ≤ τ_K^(n)) and not by its future (commitment events with τ_K^(m) > τ_K^(n)).

### 3.2 The carry-through chain

The argument proceeds in three steps, combining P11 with relational timing, the ED gradient, and bandwidth additivity.

**Step (i): Commitment events are ordered forward along γ_K.**

P11 establishes irreversibility of commitments. Combined with relational timing's continuous time axis, this gives commitment events along chain γ_K an ordered structure: each commitment event has a proper-time index n that increases monotonically. Later commitment events have larger τ_K^(n) than earlier ones. There is no "anti-chain" running backward — no chain whose commitment events have decreasing τ_K, because such a chain would require un-commitment, which P11 forbids.
...
**Step (ii): Chain proper time supplies an oriented parameterization.**

P11 combined with relational timing and the ED gradient (which together give the substrate's emergent (3,1) signature) supplies an oriented proper-time parameter τ_K for each chain. The orientation is forward in the sense that increasing τ_K corresponds to later commitment events.
...
**Step (iii): Bandwidth content is updated forward-only.**

P11 combined with bandwidth supplies a forward-only bandwidth-update rule. The bandwidth content b_K(τ_K^(n)) at commitment event n is determined by the chain's history through commitment event n — specifically by the bandwidth content at earlier commitment events τ_K^(m) for m ≤ n — but not by future commitment events with m > n.

This is the load-bearing structural fact. Why does it hold?

Because bandwidth is updated *at* commitment events. The bandwidth-update rule that determines b_K(τ_K^(n)) takes as input the bandwidth content existing at the moment the commitment event occurs, plus whatever participation-graph structure is present at that vertex. Future commitment events have not yet occurred at the moment of event n; their bandwidth content does not exist yet to be input into the update rule. The update is structurally forward-only because the rule operates on existing bandwidth content, and only past commitments have produced existing bandwidth at the moment of any present commitment.
...
### 3.3 The forward-propagator structure
...
```
δb_K(τ_K^(n)) = Σ_{m ≤ n} U_K(n, m) · δb_K^source(τ_K^(m))
```

where δb_K^source is a perturbation injected at commitment event m. The propagator vanishes for backward propagation:

```
U_K(n, m) = 0   for n < m
```

This is a structural consequence of P11 plus the carry-through arguments above. Forward propagation: the propagator can be non-zero for n ≥ m. Backward propagation: the propagator is identically zero.
...
```
δ⟨b^env⟩_vac(x) = K_vac(x - x') · δP_chain(x')
```
...
The framework's UV-FIN result (Theorem Q.8) establishes that primitive-level event-discreteness imposes a natural cutoff that's absent in continuum approximations. The framework is UV-finite without renormalization because the substrate has a discreteness scale ℓ_ED below which it is not continuum-like.

Applied to vacuum response: a delta-function kernel is incompatible with primitive-level event-discreteness. The vacuum cannot respond at resolutions finer than ℓ_ED because the substrate itself does not admit such resolutions. The vacuum-response kernel must therefore be smoothed at the ℓ_ED scale:

```
K_vac(x - x') = K_vac^prim((x - x') / ℓ_ED)
```

with K_vac^prim a bounded function that converges to the delta function only in the formal continuum limit ℓ_ED → 0. At any finite primitive-discreteness scale, the kernel has finite width.
...
### 4.2 V1 as a response kernel

The structure of V1 worth noting: V1 is a *response kernel*. It describes how the vacuum responds to perturbations, not how the vacuum correlates with itself in the absence of perturbation. This distinction matters for the directionality argument.

Standard quantum field theory has multiple two-point functions associated with the vacuum:

The Wightman function W(x, x') = ⟨0|ϕ̂(x)ϕ̂(x')|0⟩ is the unperturbed correlator. It measures intrinsic vacuum correlations between points x and x', symmetric under exchange in the standard quantum field theory.
...
These are different objects. They are mathematically related through standard QFT identities — for example, G_R(x, y) = i θ(t_x - t_y) [W(x, y) - W(y, x)] — but they are distinct as physical quantities measuring different things.
...
### 4.3 V1 is sourced by chain bandwidth content
...
The case was made explicit in §4.2: the Wightman function and the response kernel are distinct objects. The Wightman function can be symmetric while the response kernel is retarded; standard QFT has both, and they don't conflict. The framework's claim is at the response-kernel level: V1 is sourced by chain dynamics, and chain dynamics are forward-only by Section 3, so V1 inherits forward-only structure.

---

## 5. The Chain-Contribution Construction
...
### 5.1 The chain-contribution sum

Let {γ_K}_{K=1}^N index the chains in the support of the effective vacuum at spacetime point x. Each chain γ_K is a worldline parameterized by its proper time τ_K, with commitment events at indices n = 1, 2, ... and proper-time values τ_K^(n).

A chain perturbation δb_K(τ_K^(m)) at commitment index m on chain K produces a perturbation in the chain's bandwidth content at later commitment events n ≥ m via the chain's forward propagator (Section 3):
...
```
δV_vac^(K)(x) = Σ_n F_K(x; τ_K^(n)) · δb_K(τ_K^(n))
```

where F_K(x; τ) is the vacuum-coupling form-factor describing how bandwidth content at commitment event τ couples to the effective vacuum at point x. The specific functional form of F_K is inherited from continuum-level physics; the directionality argument doesn't depend on it.

### 5.2 Composition: forward-cone-only support
...
```
K_vac(x, x') = Σ_K Σ_{n ≥ m} F_K(x; τ_K^(n)) · U_K(n, m) · F_K^†(x'; τ_K^(m))
```

with the n ≥ m constraint enforced by U_K(n, m) = 0 for n < m.

The question: when does τ_K^(n) ≥ τ_K^(m) correspond to (x - x') being in the forward causal cone?

For chain K with timelike worldline γ_K, the proper-time parameter τ_K is monotone increasing along the forward time orientation of the chain. P11 supplies this orientation: increasing τ_K corresponds to later commitment events, decreasing τ_K to earlier. All chains share this orientation — there is no anti-chain running backward in time, because P11 forbids un-commitment for every chain in the inventory.

P11 is universal across the chain ensemble. Every chain has the same forward time orientation in its proper-time parameterization. When chain K contributes to V1 with τ_K^(n) ≥ τ_K^(m), the spacetime points (x_n^K, x_m^K) on the chain's worldline satisfy (x_n^K - x_m^K) is in the forward causal cone of γ_K. Since γ_K is timelike with universal forward orientation, this maps to (x - x') being in the forward causal cone in any frame where γ_K's velocity is forward-pointing — i.e., in every Lorentz frame consistent with the universal P11-supplied time orientation.

Conclusion:

```
K_vac(x, x') ≠ 0  ⟹  (x - x') is in the forward causal cone
```

V1 has forward-cone-only support. V1 is retarded.

### 5.3 Symmetric V1 cannot be constructed
...
```
K_vac^sym(x, x') = G(σ(x, x') / ℓ_ED²)
```

without the forward-cone restriction θ(t - t').
...
But symmetric V1 cannot be constructed from the chain-contribution sum. For a symmetric V1 to arise from chain contributions, the kernel sum would need to admit terms with τ_K^(n) < τ_K^(m) — that is, backward chain propagation. By P11, U_K(n, m) = 0 for n < m. There is no such term.

A symmetric V1 cannot be sourced by forward-only chain dynamics. To achieve symmetric structure, V1 would need a non-chain-sourced component — a kernel-level contribution that bypasses chain dynamics. But §4.2 established that V1 has no non-chain-sourced contribution as a response kernel; non-chain contributions are intrinsic vacuum correlations (Wightman-type), not response.
...
### 5.4 Advanced V1 is directly inconsistent

The other alternative, advanced V1, has backward-cone-only support:

```
K_vac^adv(x, x') = θ(t' - t) · G(σ / ℓ_ED²)
```
...
### 5.5 Robustness: the R1-bypass redundancy

The argument has a structural redundancy worth noting. The directionality of V1 was established through the vacuum-coupling cascade: chain dynamics → chain bandwidth content → vacuum-coupled bandwidth memory. But the framework's vacuum-coupled bandwidth memory (called N1-E in the framework's catalogue) has a second route to forward-only directionality, independent of the V1-cascade argument.
...
P11 commitment irreversibility (the seed direction-bearing primitive).

Chain dynamics forced to be forward-only by P11 plus relational timing plus the ED gradient plus bandwidth (Section 3).

V1's existence with finite-width structure forced by event-discreteness plus UV-finiteness (Section 4).

V1 as a response kernel sourced by chain bandwidth content via vacuum coupling (Section 4).

The chain-contribution sum admitting only forward-cone contributions (Section 5).

The non-existence of non-chain-sourced contributions to V1 as a response kernel (Section 4).

The refutation of symmetric V1 by non-construction (Section 5.3).

The refutation of advanced V1 by direct inconsistency with P11 (Section 5.4).
...
## 6. The Continuum-Approximation Picture
...
### 6.1 The response-kernel / correlator distinction
...
```
G_R(x, y) = i θ(t_x - t_y) [W(x, y) - W(y, x)]
```
...
### 7.1 The Second Law is not derived
...
### 7.2 The radiation arrow is not addressed
...
### 7.5 What the framework does claim, plainly
...
Primitives (micro-events, participation, bandwidth, chains, commitment with P11 irreversibility, relational timing, ED gradient) → forward-only chain bandwidth dynamics (P11 plus carry-through) → V1 exists with finite-width structure (event-discreteness plus UV-finiteness) → V1 is a response kernel sourced by chain bandwidth content (effective-vacuum factorization) → chain-contribution sum admits only forward-cone contributions → V1 has forward-cone-only support → kernel-level arrow of time forced.
...
- Boltzmann, L. *Vorlesungen über Gastheorie.* Leipzig: J. A. Barth, 1896-1898. (Statistical mechanics origin of the thermodynamic arrow.)
- Albert, D. Z. *Time and Chance.* Cambridge: Harvard University Press, 2000. (Modern philosophical treatment of the past hypothesis.)
- Price, H. *Time's Arrow and Archimedes' Point: New Directions for the Physics of Time.* Oxford University Press, 1996. (Survey of physical and philosophical approaches to time asymmetry.)
- Zeh, H. D. *The Physical Basis of the Direction of Time.* Berlin: Springer, 5th ed., 2007. (Comprehensive review of physical arrows of time.)
- Proxmire, A. *Structurally FORCED Non-Markovian Structures.* Arc N Stage N.2 evaluation memo. April 2026.
- Proxmire, A. *FORCED Evaluation of the Microscopic Arrow of Time.* Arc B Stage B.2 evaluation memo. April 2026.
- Proxmire, A. *Q.8 Memo 02 — Vacuum-Level Commitment and Gauge-Quotient Audit.* Arc Q substage memo. April 2026.
