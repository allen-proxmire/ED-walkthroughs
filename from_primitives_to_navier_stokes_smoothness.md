...
The R1 mechanism is a real, form-forced regularizing infrastructure. The substrate-cutoff stabilization −κμ_V1 ℓ_P² ∇⁴v_i arises from V1's finite-width vacuum kernel under multi-scale expansion, with sign forced positive by V1's positive-smoothing-kernel structure.
...
**P4** — Mobility capacity bound: M(ρ_max) = 0 with M(ρ) > 0 for ρ < ρ_max, encoding finite packing capacity.
...
Hydrodynamic regime: R_cg much larger than the mean free path λ_mfp.
...
```
component-wise mobility channel → μ∇²v_i
```
...
```
ρ Dv_i/Dt = −∂_i p + μ∇²v_i − κμ_V1 ℓ_P² ∇⁴v_i + ρf^ext_i
```

The R1 term −κμ_V1 ℓ_P² ∇⁴v_i is the structural shadow of ED's substrate UV cutoff in the continuum momentum equation.
...
```
multi-scale expansion → −κμ_V1 ℓ_P² ∇⁴v_i
```
...
**Magnitude is value-INHERITED.** The coefficient κμ_V1 depends on V1's specific G-function profile, which is inherited from material-specific substrate physics rather than fixed by the canon at primitive level.
...
```
ρ ∂_t v_i = μ∇²v_i − κμ_V1 ℓ_P² ∇⁴v_i − ∂_i p,    ∇·v = 0
```
...
The components are: viscous diffusion μ∇²v_i (standard kinematic-viscosity-class diffusion, P1-class mobility channel applied component-wise), R1 stabilization −κμ_V1 ℓ_P² ∇⁴v_i (form-forced higher-derivative regularization from V1's finite-width vacuum kernel), and pressure −∂_i p (Lagrange multiplier enforcing incompressibility).
...
```
L(t) = (1/2) ‖∇v(t)‖²₂ = (1/2) ∫_{ℝ³} ∂_j v_i ∂_j v_i dV
```
...
```
∫ ∂_j v_i ∂_j(−κμ_V1 ℓ_P² ∇⁴v_i) dV = −κμ_V1 ℓ_P² ∫ |∇³v|² dV ≤ 0
```
...
```
dL/dt |_{ED-only NS} = −ν ∫ |∇²v|² dV − κμ_V1 ℓ_P² ∫ |∇³v|² dV ≤ 0
```
...
```
ρ ∂_t v_i + ρ(v·∇)v_i = μ∇²v_i − κμ_V1 ℓ_P² ∇⁴v_i − ∂_i p,    ∇·v = 0
```
...
Computing dL/dt term-by-term, three of four contributions are unchanged from the ED-only case: viscous diffusion contributes −ν‖∇²v‖²₂ ≤ 0; R1 contributes −κμ_V1 ℓ_P² ‖∇³v‖²₂ ≤ 0; pressure contributes zero by incompressibility. The new contribution comes from advection.
...
The vortex-stretching contribution depends on the alignment of vorticity ω with the strain-rate tensor's eigenvectors. The strain S has three real eigenvalues λ_1 ≥ λ_2 ≥ λ_3 with λ_1 + λ_2 + λ_3 = 0 by incompressibility (tracelessness of S follows from ∇·v = 0).

Three regimes:

**Vorticity aligned with λ_1-eigenvector** (positive eigenvalue, by tracelessness λ_1 ≥ 0). The integrand ω·Sω is positive locally. Vortex stretching amplifies vorticity — the canonical signature of 3D turbulent enstrophy production.

**Vorticity aligned with λ_3-eigenvector** (negative eigenvalue, by tracelessness λ_3 ≤ 0). The integrand is negative locally. Vortex compression diminishes vorticity.
...
```
dL/dt = −ν‖∇²v‖²₂  −  κμ_V1 ℓ_P² ‖∇³v‖²₂  +  0  +  ∫ ω·Sω dV · (const)
        ≤ 0           ≤ 0                      pressure   indefinite-sign
```
...
```
M_ijm(k) = −i k_j P_im(k),    P_im(k) = δ_im − k_i k_m / k²
```
...
|-----------|--------|------|--------|
| Viscous diffusion −ν‖∇²v‖² | Standard NS | Dissipative (≤ 0) | Standard |
| R1 stabilization −κμ_V1 ℓ_P²‖∇³v‖² | ED canonical | Dissipative (≤ 0); sign FORCED | Form-FORCED ED architecture |
...
**Intermediate Path C verdict.** Event Density's architectural canon contains a real Clay-NS-relevant regularizing mechanism. The R1 mechanism — the form-forced −κμ_V1 ℓ_P² ∇⁴v_i stabilization arising from V1's finite-width vacuum kernel under multi-scale expansion — combined with standard viscous diffusion produces, in the counterfactual ED-only NS lacking the advective term, a strictly monotonically-decaying gradient-norm Lyapunov L = (1/2)‖∇v‖²₂. By standard parabolic-regularity theory, ED-only NS has global smooth solutions on ℝ³ for smooth, finite-energy initial data.
