...
```
ds²_substrate ≈ −ρ² a² · dt² + dρ² + (transverse pieces)
```
...
```
ds²_substrate ≈ ρ² a² · dτ² + dρ² + (transverse pieces)
```
...
```
G(-iβ + Δt) = G(Δt)*  (with appropriate hermitian conjugation)
```
...
For a hypothetical observer accelerated at $a = c²/L$ (where $L$ is the proper distance to the Rindler horizon), the Unruh temperature is $T_U = \hbar c/(2\pi L k_B)$ — inversely proportional to the proper distance to the horizon.
