...
The quantized Hall drift is FORCED at the substrate level whenever a minimal ED-channel (light) propagates through a periodic rule-type substrate with non-zero global rule-type curvature. The mechanism is structural:
...
```
∂_t ρ = -i[H, ρ] + Σ_α [L_α ρ L_α† - (1/2){L_α† L_α, ρ}]    (set ∂_t ρ = 0 for steady state)
```
...
```
H_synth = -t_1 Σ_m (|m+1⟩⟨m| + h.c.) - t_2 Σ_m (e^{iφ} |m+2⟩⟨m| + h.c.).
```
...
```
ψ_k(2n) = e^{ikna} u^A_k,    ψ_k(2n+1) = e^{ikna} u^B_k    (for some k-dependent amplitudes u^A_k, u^B_k)
```

equivalently, the Bloch form on the two-sublattice synthetic lattice:

```
|ψ_k⟩ = Σ_n e^{ikna} (u^A_k |A_n⟩ + u^B_k |B_n⟩).
```
...
```
|ψ_k⟩ = e^{i k·n_a} (u^A_k |A⟩ + u^B_k |B⟩),    k = (k_x, k_y) ∈ 𝕋²,
```
...
```
H(k) = h_x(k) σ_x + h_y(k) σ_y + h_z(k) σ_z + ε(k) I,
```
...
```
h_x(k) + i h_y(k) = -t_1 Σ_{δ} e^{i k·δ}    (sum over NN vectors δ from A to B)
h_z(k)            = M - 2 t_2 sin(φ) Σ_{δ_NNN} sin(k · δ_NNN)
ε(k)              = -2 t_2 cos(φ) Σ_{δ_NNN} cos(k · δ_NNN),
```
...
```
u_-(k) = (sin(θ/2), -e^{iφ_h} cos(θ/2))^T,    u_+(k) = (cos(θ/2), e^{iφ_h} sin(θ/2))^T,
```
...
```
ĥ(k) = (sin θ cos φ_h, sin θ sin φ_h, cos θ).
```
...
```
∂_{k_i} |u_-⟩ = ((1/2) cos(θ/2) ∂_{k_i}θ, -(1/2)(-e^{iφ_h} sin(θ/2)) ∂_{k_i}θ - i e^{iφ_h} cos(θ/2) ∂_{k_i}φ_h)^T
            = ((1/2) cos(θ/2) ∂_i θ, (1/2) e^{iφ_h} sin(θ/2) ∂_i θ - i e^{iφ_h} cos(θ/2) ∂_i φ_h)^T.
```

Compute $\langle u_- | \partial_{k_i} u_-\rangle$:

```
⟨u_- | ∂_i u_-⟩ = sin(θ/2)·(1/2)cos(θ/2)∂_i θ
                + (-e^{-iφ_h}cos(θ/2))·[(1/2)e^{iφ_h}sin(θ/2)∂_i θ - i e^{iφ_h} cos(θ/2) ∂_i φ_h]
              = (1/2)sin(θ/2)cos(θ/2)∂_i θ - (1/2)cos(θ/2)sin(θ/2)∂_i θ + i cos²(θ/2) ∂_i φ_h
              = i cos²(θ/2) ∂_i φ_h.
```
...
```
A_-(k) = i · ⟨u_- | ∇_k u_-⟩ = -cos²(θ(k)/2) ∇_k φ_h(k).
```

Equivalently, using $\cos^2(\theta/2) = (1 + \cos\theta)/2$:

```
A_-(k) = -(1/2)(1 + ĥ_z(k)) ∇_k φ_h(k).
```
...
```
∬_{𝕋²} Ω_-(k) d²k = -(1/2) ∬_{𝕋²} ĥ · (∂_{k_x} ĥ × ∂_{k_y} ĥ) d²k = -(1/2) · 4π · (degree of ĥ),
```
...
```
Ω_-(k) → ∂_{k_x}(A_-^y - ∂_{k_y}χ) - ∂_{k_y}(A_-^x - ∂_{k_x}χ) = Ω_-(k)
```
...
```
ℏ k̇ = F,
ℏ ẋ_i = ∂E_n(k)/∂k_i - ε_{ij} F_j Ω_n(k),
```
...
```
⟨ẋ_i⟩ = (∂x_{0,i}/∂k_j)(dk_j/dt) + (∂A_n^i/∂k_j)(dk_j/dt)
      = (∂E_n/∂k_i) + (∂A_n^i/∂k_j)·F_j/ℏ
      = (1/ℏ) ∂E_n/∂k_i + F_j/ℏ · (∂_j A_n^i)
      = (1/ℏ)[∂E_n/∂k_i - F_j Ω_n^{ji}],
```
...
```
ℏ ẋ = ∂E_n/∂k_x,           (longitudinal)
ℏ ẏ = ∂E_n/∂k_y - F · Ω_n. (transverse, with anomalous-velocity term)
```
...
```
Δy = ∫_0^T ẏ dt = (1/ℏ) ∫_0^T [∂E_n/∂k_y - F Ω_n] dt.
```
...
```
Δy = -(F/ℏ) ∫_0^T Ω_n(k(t)) dt = -(F/ℏ) · (ℏ/F) ∫_{-π/a}^{π/a} Ω_n(k_x, k_y) dk_x = -∫_{-π/a}^{π/a} Ω_n(k_x, k_y) dk_x.
```

If we further integrate over all $k_y$ values populated by the chain (e.g., a uniform fill of the lower band):

```
⟨Δy⟩_band = -(a/(2π)) ∫_{-π/a}^{π/a} dk_y ∫_{-π/a}^{π/a} dk_x Ω_n(k_x, k_y) = -(a/(2π)) · ∬_𝕋² Ω_n d²k = -a · C_n.
```
...
```
∂_t ρ = -i [H_sys, ρ] + Σ_α [L_α ρ L_α† - (1/2){L_α† L_α, ρ}].
```
...
```
|u_n^{(2)}(k)⟩ = e^{iχ_{12}(k)} |u_n^{(1)}(k)⟩    for k ∈ U_{12},
```
...
```
A_n^{(2)}(k) = A_n^{(1)}(k) - ∇_k χ_{12}(k)    on U_{12}.
```
...
```
∮_{∂U_1} (A_n^{(2)} - A_n^{(1)}) · dk = ∮_{∂U_1} (-∇χ_{12}) · dk = -[χ_{12}(end) - χ_{12}(start)].
```
...
```
χ_{12}(end) - χ_{12}(start) = 2π m,    m ∈ ℤ,
```
...
```
∬_{𝕋²} Ω_n = ∮_{∂U_1} A_n^{(1)} · dk + ∮_{∂U_2} A_n^{(2)} · dk.
```
...
```
∬_{𝕋²} Ω_n = ∮_{∂U_1} A_n^{(1)} · dk - ∮_{∂U_1} A_n^{(2)} · dk
           = ∮_{∂U_1} (A_n^{(1)} - A_n^{(2)}) · dk
           = ∮_{∂U_1} ∇χ_{12} · dk
           = 2π m,    m ∈ ℤ.
```
