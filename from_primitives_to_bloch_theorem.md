...
*A walkthrough-grade ED Arc deriving Bloch's theorem and band structure for a light-like ED-channel propagating through a periodic rule-type substrate. Fully self-contained: all required math is derived inside this document.*
...
1. **Bloch's theorem**: in a periodic rule-type substrate, the eigenchannels of the chain's evolution operator have the form
   ...
   ```
   ψ_k(x) = e^{ikx} u_k(x),    u_k(x+a) = u_k(x),
   ```
...
```
(T_a V ψ)(x) = (Vψ)(x - a) = V(x - a) ψ(x - a) = V(x) ψ(x - a)    (using V(x-a) = V(x), i.e., periodicity)
             = V(x) (T_a ψ)(x) = (V T_a ψ)(x).
```
...
```
(T_a ψ)(x) = ψ(x - a) = e^{ik(x-a)} u_k(x - a) = e^{-ika} · e^{ikx} u_k(x - a).
```
...
```
ψ_{n,k}(x) = e^{ikx} u_{n,k}(x),    u_{n,k}(x + a) = u_{n,k}(x),    k ∈ 𝒷,
```
...
```
[-ℏ²/(2m) ∂²/∂x² + V(x)] [e^{ikx} u_k(x)]  =  E e^{ikx} u_k(x).
```

Compute the kinetic term:

```
∂_x [e^{ikx} u_k] = e^{ikx} [ik u_k + ∂_x u_k],
∂²_x [e^{ikx} u_k] = e^{ikx} [(ik)² u_k + 2(ik) ∂_x u_k + ∂²_x u_k]
                    = e^{ikx} [-k² u_k + 2ik ∂_x u_k + ∂²_x u_k].
```

So

```
-ℏ²/(2m) ∂²_x [e^{ikx} u_k] = e^{ikx} [(-ℏ²/(2m)) (∂_x + ik)² u_k]
                              = e^{ikx} · ℏ²/(2m) · (-(∂_x + ik)²) u_k.
```

Setting this in the eigenvalue equation and dividing by $e^{ikx}$:

```
[ℏ²/(2m) (-(∂_x + ik)²) + V(x)] u_k(x)  =  E u_k(x),
```

equivalently

```
H_k u_k(x) = E_k u_k(x),    where    H_k ≡ -(ℏ²/(2m)) (∂_x + ik)² + V(x).
```
...
```
ΔE_{n,n+1} = min_{k∈𝒷} E_{n+1}(k) - max_{k∈𝒷} E_n(k).
```
...
```
ψ_k(x) = e^{i k·x} u_k(x),    u_k(x + a_j) = u_k(x)   for all j.
```
...
```
A_n(k) ≡ i ⟨u_{n,k} | ∂_k u_{n,k}⟩_cell.
```
...
```
⟨∂_k u_{n,k} | u_{n,k}⟩_cell + ⟨u_{n,k} | ∂_k u_{n,k}⟩_cell = 0,
```

so

```
⟨u_{n,k} | ∂_k u_{n,k}⟩_cell = -⟨∂_k u_{n,k} | u_{n,k}⟩_cell = -⟨u_{n,k} | ∂_k u_{n,k}⟩_cell^*.
```
...
```
Ã_n(k) = i ⟨ũ_{n,k} | ∂_k ũ_{n,k}⟩
       = i ⟨u_{n,k} | e^{-iχ} ∂_k [e^{iχ} u_{n,k}]⟩
       = i ⟨u_{n,k} | e^{-iχ} [i (∂_k χ) e^{iχ} u_{n,k} + e^{iχ} ∂_k u_{n,k}]⟩
       = -⟨u_{n,k}|u_{n,k}⟩ ∂_k χ + i ⟨u_{n,k} | ∂_k u_{n,k}⟩
       = A_n(k) - ∂_k χ(k).
```
...
```
Ω̃_n = ∂_{k_x}(A_n^y - ∂_{k_y}χ) - ∂_{k_y}(A_n^x - ∂_{k_x}χ)
     = (∂_{k_x} A_n^y - ∂_{k_y} A_n^x) - (∂_{k_x}∂_{k_y}χ - ∂_{k_y}∂_{k_x}χ)
     = Ω_n - 0 = Ω_n,
```
